...
class Block(nn.Module):
    ...
    def __init__(
            self, L, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_dim=768, depth=12, num_heads=8, mlp_ratio=4., qkv_bias=True, init_values=None,
        ...
        self.encoder_blocks1 = nn.Sequential(*[
            block_fn(
                L=num_patches + 1, dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
        ...
        # self.norm = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()
        # self.encoder_blocks2 = nn.Sequential(*[
        #     block_fn(
        #         dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
        #         drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
        #     for i in range(int(depth/2))])
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.encoder_blocks, x)
        else:
