...
def read_pose_file(filepath):
    ...
    save_to = os.path.join('./data/WLASL/fetures', vid)
    ...
    try:
        ...
        save_to = os.path.join('./WLASL/data/fetures', vid)
...
class Sign_Dataset(Dataset):
    def __init__(self, index_file_path, split, pose_root, sample_strategy='rnd_start', num_samples=25, num_copies=4,
                 img_transforms=None, video_transforms=None, test_index_file=None, transforms=None):
    ...
    def __getitem__(self, index):
        ...
        raw_data = self._load_poses(video_id, frame_start, frame_end, self.sample_strategy, self.num_samples)
        y = gloss_cat

        if self.video_transforms:
            sample = self.video_transforms(sample)

        raw_x = raw_data[:,::2].unsqueeze(0)
        raw_y = raw_data[:,1::2].unsqueeze(0)
        raw_data = torch.cat((raw_x,raw_y),dim=0).contiguous().float() # NCJT
        return raw_data, y
    ...
    def _make_dataset(self, index_file_path, split):
        ...
        for gloss_entry in content:
            ...
            gloss_cat = datasetutil.labels2cat(self.label_encoder, [gloss])[0]
